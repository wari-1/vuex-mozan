<template>
  <div class="distinct-component-ct">
    ...
    >
      <el-option v-for="(item, index) in provinceArr" :key="index" :label="item" :value="item"></el-option>
    </el-select>
    <el-select
      :style="{
          width: selectWidth + unit,
      ...
      @change="cityChange"
    >
      <el-option v-for="(item, index) in computingCitys" :key="index" :lable="index" :value="index"></el-option>
    </el-select>
    <el-select
      ...
      @change="areaChange"
    >
      <el-option v-for="(item, index) in computingAreas" :label="item" :key="index" :value="item"></el-option>
    </el-select>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    computingAreas() {
      return this.provinceSelect && this.citySelect
        ? getAreas(this.provinceSelect, this.citySelect)
        : [];
    }
  },
  watch: {
